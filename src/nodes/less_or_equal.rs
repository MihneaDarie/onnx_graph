...
#[derive(Default)]
pub struct LessOrEqualNode<T: Default> {
    a: u64,
    b: u64,
...
impl<T: Default + 'static> Node<T> for LessOrEqualNode<T> {
    ...
    fn execute(&self, omap: &mut TensorMap) -> anyhow::Result<()> {
        ...
        crate::debug_check_tensors!("LessOrEqualNode", a => self.a, b => self.b, o => self.o);
        if let (Some(a), Some(b), Some(out)) = (a, b, o) {
            a.less_or_equal_op(b, out)?;
        }
        Ok(())
    }
