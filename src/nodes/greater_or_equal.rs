...
impl<T: Default> GreaterOrEqualNode<T> {
    pub fn new(elem: &OnnxOperation) -> Self {
        ...
        greater_or_equal.add_inputs(aid, bid);
        let oid = hash_string(&elem.outputs()[0]);
        greater_or_equal.add_outputs(oid);
        greater_or_equal
    }
...
impl<T: Default + 'static> Node<T> for GreaterOrEqualNode<T> {
    ...
    fn execute(&self, omap: &mut TensorMap) -> anyhow::Result<()> {
        ...
        crate::debug_check_tensors!("GreaterOrEqualNode", a => self.a, b => self.b, o => self.o);
        if let (Some(a), Some(b), Some(out)) = (a, b, o) {
            a.greater_or_equal_op(b, out)?;
        }
        Ok(())
    }
