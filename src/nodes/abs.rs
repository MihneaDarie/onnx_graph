...
impl<T: Default + 'static> Node<T> for AbsNode<T> {
    ...
    fn execute(&self, omap: &mut TensorMap) -> anyhow::Result<()> {
        ...
        let x = x.map(|val| &*val);
        crate::debug_check_tensors!("AbsNode", x => self.x, o => self.o);
